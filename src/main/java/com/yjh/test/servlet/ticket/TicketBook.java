    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            ...
            if (ticketList.size() == 0) {
                ...
                result.setStatus(false);
                result.setReasons("该票已售出或已被预定");
            } else {
                PlayService playService = new PlayService();
                int playID = ticketList.get(0).getPlayID();
                List<Play> playList = playService.quary(playID);
                if (ticketService.insert(id, playID, playList.get(0).getPrice(), TicketStatus.BOOK, userID) > 0) {
                    result.setStatus(true);
                    result.setReasons("订票成功");
                    ticketList.get(0).setStatus(TicketStatus.BOOK);
                    result.setData(ticketList.get(0));
                } else {
                    result.setStatus(false);
                    result.setReasons("订票失败");
                }
            }
